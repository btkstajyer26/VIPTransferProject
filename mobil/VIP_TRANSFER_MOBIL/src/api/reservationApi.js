...
export async function getReservationPricePreview({ reservationData, requiresAuth = false } = {}) {
  if (!reservationData || typeof reservationData !== 'object' || Array.isArray(reservationData)) {
    throw createValidationError('Fiyat hesaplama bilgileri gerekli.');
  }

  const previewData = {
    pickupLat: reservationData.pickupLat,
    pickupLon: reservationData.pickupLon,
    dropoffLat: reservationData.dropoffLat,
    dropoffLon: reservationData.dropoffLon,
    vehicleId: reservationData.vehicleId,
    scheduledTime: reservationData.scheduledTime,
    campaignCode: reservationData.campaignCode,
  };

  try {
    return await apiClient.request(`${RESERVATIONS_PATH}/price-preview`, {
      method: 'POST',
      body: previewData,
      requiresAuth,
    });
  } catch (error) {
    rethrowApiError(error, 'Fiyat ayrıntıları hesaplanamadı.');
  }
}
